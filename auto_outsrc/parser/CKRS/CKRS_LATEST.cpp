...
void extract(CharmList & mpk, CharmList & msk, string & id, ZR & bf1, CharmList & skBlinded)
{
    string idBlinded;
    GT omega;
    G1 g;
    ...
    G2 d5 = group.init(G2_t);
    G2 d5Blinded = group.init(G2_t);
    bf1 = group.random(ZR_t);
    idBlinded = id;
    ...
    hID = stringToInt(group, idBlinded, 5, 32);
    //;
    for (int y = 0; y < n; y++)
    {
    ...
    d0Blinded = group.exp(d0, group.div(1, bf1));
    ...
    d1Blinded = group.exp(d1, group.div(1, bf1));
    d2 = group.mul(group.exp(halpha, t1), group.exp(hashID2r1, t1));
    d2Blinded = group.exp(d2, group.div(1, bf1));
    hashID2r2 = group.exp(hashID, group.neg(r2));
    d3 = group.exp(hashID2r2, t4);
    d3Blinded = group.exp(d3, group.div(1, bf1));
    d4 = group.exp(hashID2r2, t3);
    d4Blinded = group.exp(d4, group.div(1, bf1));
    d5 = group.mul(group.exp(h, t1), group.exp(h, t2));
    d5Blinded = group.exp(d5, group.div(1, bf1));
...
void transform(CharmList & skBlinded, CharmList & ct, GT & transformOutputList)
{
    string idBlinded;
    G2 d0Blinded;
    G2 d1Blinded;
...
void decout(GT & transformOutputList, ZR & bf1, GT & M)
{
    GT cpr = group.init(GT_t);
    GT result = group.init(GT_t);
    cpr = transformOutputList[1].getGT();
    result = group.exp(transformOutputList[0].getGT(), bf1);
