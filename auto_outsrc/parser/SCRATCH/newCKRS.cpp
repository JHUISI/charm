...
void extract(CharmList & mpk, CharmList & msk, string & id, ZR & blindingFactord0Blinded, CharmList & skBlinded)
{
    string idBlinded;
    ...
    d1Blinded = group.exp(d1, group.div(1, blindingFactord0Blinded));
    d2 = group.mul(group.exp(halpha, t1), group.exp(hashID2r1, t1));
    d2Blinded = group.exp(d2, group.div(1, blindingFactord0Blinded));
    hashID2r2 = group.exp(hashID, group.neg(r2));
    d3 = group.exp(hashID2r2, t4);
    d3Blinded = group.exp(d3, group.div(1, blindingFactord0Blinded));
    d4 = group.exp(hashID2r2, t3);
    d4Blinded = group.exp(d4, group.div(1, blindingFactord0Blinded));
...
void decout(CharmList & sk, CharmList & ct, CharmList & transformOutputList, ZR & blindingFactord0Blinded, GT & M)
{
    string id;
    G2 d0;
    G2 d1;
    ...
    c4 = ct[4].getG1();
    cpr = ct[5].getGT();
    result = group.exp(transformOutputList[0].getGT(), blindingFactord0Blinded);
