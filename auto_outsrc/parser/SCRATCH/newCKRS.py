...
def extract(mpk, msk, id):
    ...
    d1Blinded = (d1 ** (1 / blindingFactord0Blinded))
    d2 = ((halpha ** t1) * (hashID2r1 ** t1))
    d2Blinded = (d2 ** (1 / blindingFactord0Blinded))
    hashID2r2 = (hashID ** -r2)
    d3 = (hashID2r2 ** t4)
    d3Blinded = (d3 ** (1 / blindingFactord0Blinded))
    d4 = (hashID2r2 ** t3)
    d4Blinded = (d4 ** (1 / blindingFactord0Blinded))
    ...
    output = (blindingFactord0Blinded, skBlinded)
    return output
...
def decout(sk, ct, transformOutputList, blindingFactord0Blinded):
    id, d0, d1, d2, d3, d4 = sk
    c0, c1, c2, c3, c4, cpr = ct
    result = (transformOutputList[0] ** blindingFactord0Blinded)
    M = (cpr * result)
    output = M
    return output
