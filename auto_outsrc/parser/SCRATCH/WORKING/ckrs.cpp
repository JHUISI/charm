...
void extract(CharmList & mpk, CharmList & msk, string & id, ZR & blindingFactor0Blinded, CharmList & skBlinded)
{
    string idBlinded;
    ...
    for (int y = 0; y < n; y++)
    {
        reservedVarName1 = group.exp(hl[y].getG2(), hID[y].getG2());
        reservedVarName0 = group.mul(reservedVarName0, reservedVarName1);
    }
    hashIDDotProd = reservedVarName0;
    hashID = group.mul(hl[0].getG2(), hashIDDotProd);
...
void encrypt(CharmList & mpk, GT & M, string & id, CharmList & ct)
{
    ...
    for (int y = 0; y < n; y++)
    {
        reservedVarName3 = group.exp(gl[y].getG1(), hID1[y].getG1());
        reservedVarName2 = group.mul(reservedVarName2, reservedVarName3);
    }
    hashID1DotProd = reservedVarName2;
    hashID1 = group.mul(gl[0].getG1(), hashID1DotProd);
...
void transform(CharmList & sk, CharmList & ct, CharmList & transformOutputList)
{
    ...
    c4 = ct[4].getG1();
    cpr = ct[5].getGT();
    transformOutputList.insert(0, group.mul(group.pair(c0, d0), group.mul(group.pair(c1, d1), group.mul(group.pair(c2, d2), group.mul(group.pair(c3, d3), group.pair(c4, d4))))));
    result = transformOutputList[0].getGT();
    return;
}

void decout(CharmList & sk, CharmList & ct, CharmList & transformOutputList, ZR & blindingFactor0Blinded, GT & M)
{
    string id;
    G2 d0;
    G2 d1;
