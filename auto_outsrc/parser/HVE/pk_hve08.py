...
class HVE08:
    ...
    def keygen(self, pk, msk, yVector):
        ...
        if (n != yVectorLen):
            sys.exit("pk_hve08.py:  length of yVector passed in to keygen is unequal to n passed in to setup.")
        ...
        for i in range(0, yVectorLen):
            if (yVector[i] == 0):
                ...
                sys.exit("pk_hve08.py:  one of the yVector elements is not 0, 1, or 2 (only allowable values).")
    ...
    def encrypt(self, M, xVector, pk):
        ...
        if (n != xVectorLen):
            sys.exit("pk_hve08.py:  the length of the xVector passed in to encrypt is unequal to the n value passed in to setup.")
        ...
        for i in range(0, n):
            if (xVector[i] == 0):
                ...
                sys.exit("pk_hve08.py:  one of the xVector elements passed into encrypt is not either 0 or 1 (only allowable values).")
    ...
    def decrypt(self, CT, sk):
        ...
        if ( (n != len(LVector)) or (n != len(XVector)) or (n != len(WVector)) ):
            sys.exit("pk_hve08.py:  lengths of the vectors passed to decrypt are unequal in at least one case.")
