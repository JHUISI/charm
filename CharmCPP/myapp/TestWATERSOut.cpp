...
void Waters09::keygen(CharmList & pk, CharmList & msk, CharmListStr & S, ZR & bf0, CharmList & skBlinded)
{
    ...
    string z;
    CharmListG1 Kl;
    CharmListZR blindingFactorKlBlinded;
    ...
    for (int y_var = 0; y_var < Kl_len; y_var++)
    {
        ...
        blindingFactorKlBlinded.insert(y, bf0);
        KlBlinded.insert(y, group.exp(Kl[y], group.div(1, blindingFactorKlBlinded[y])));
    }
...
void Waters09::transform(CharmList & pk, CharmList & skBlinded, CharmListStr & S, CharmList & ct, CharmList & transformOutputList, int & Y)
{
    string policy_str;
    GT C;
    G1 Cpr;
    ...
    transformOutputList.insert(1, C);
    policy = util.createPolicy(policy_str);
    ...
    for (int y = 0; y < Y; y++)
    {
        ...
        transformOutputList.insert(10+y, group.mul(group.pair(group.exp(Cn[yGetStringSuffix], coeff[yGetStringSuffix]), LBlinded), group.pair(group.exp(KlBlinded[yGetStringSuffix], coeff[yGetStringSuffix]), Dn[yGetStringSuffix])));
        reservedVarName1 = transformOutputList[10+y].getGT();
    }
    transformOutputList.insert(0, group.pair(Cpr, KBlinded));
    result0 = transformOutputList[0].getGT();
    return;
}

void Waters09::decout(CharmList & pk, CharmListStr & S, CharmList & transformOutputList, ZR & bf0, int Y, GT & M)
{
    GT C = group.init(GT_t);
    ...
    C = transformOutputList[1].getGT();
    //;
    for (int y = 0; y < Y; y++)
    {
		//NOP;
        reservedVarName1 = group.exp(transformOutputList[10+y].getGT(), bf0);
        reservedVarName0 = group.mul(reservedVarName0, reservedVarName1);
    }
    A = reservedVarName0;
    result0 = group.exp(transformOutputList[0].getGT(), bf0);
