...
def genValidSignature(message, index, g, X, Y, x, y):
    sig = cl.sign(x, y, message)
    (a, b, c) = sig
    assert cl.verify(X, Y, g, message, a, b, c)
    return sig
...
def generate_signatures_main(argv, same_signer=True):
    ...
    # 2. serialize the pk's
        pick_mpk = objectToBytes({ 'g':g, 'X':X, 'Y':Y }, group)
        f_mpk.write(pick_mpk)
        f_mpk.close()
    else:
    # uncomment for different signers
        pklist = {}
        sklist = {}
        for z in range(0, numValidMessages):
            (X, Y, x, y) = cl.keygen(g) # returns X, Y, x, y
            pklist[z] = [X, Y]
            sklist[z] = [x, y]
...
#    invalidOutputDict[0]['pk'] = 'pk.charmPickle'
    ...
    genOutputDictFile(numValidMessages, messageSize, 'mpk.charmPickle', 'pk.charmPickle', validOutputDict, validOutputDictName, '_ValidMessage.pythonPickle', '_ValidSignature.charmPickle', True, g, X, Y, x, y)
    genOutputDictFile(numInvalidMessages, messageSize, 'mpk.charmPickle', 'pk.charmPickle', invalidOutputDict, invalidOutputDictName, '_InvalidMessage.pythonPickle', '_InvalidSignature.charmPickle', False, g, X, Y, x, y)
    return
...
#            print("cycle is ", cycle)
            ...
            X  = sigsDict[0]['mpk'][bodyKey]['X']
            Y  = sigsDict[0]['mpk'][bodyKey]['Y']
#            if same_signer:
...
#                pklist = [ _list[i] for i in range(0, N) ]
            ...
            alist = [ sigsDict[i]['sig'][bodyKey][0] for i in range(0, N) ]
            blist = [ sigsDict[i]['sig'][bodyKey][1] for i in range(0, N) ]
            clist = [ sigsDict[i]['sig'][bodyKey][2] for i in range(0, N) ]

            startTime = time.clock()
            incorrectSigIndices = cl.batchverify(g, alist, Mlist, clist, blist, Y, X, [])
            endTime = time.clock()
            ...
            if (incorrectSigIndices != []):
                sys.exit("Batch verification returned invalid signatures: " + str(incorrectSigIndices))
            ...
            startTime = time.clock()
            incorrectSigIndices = cl.indivverify(g, alist, Mlist, clist, blist, Y, X, [])
            endTime = time.clock()
            ...
            if (incorrectSigIndices != []):
                sys.exit("Ind. verification returned invalid signatures: " + str(incorrectSigIndices))
