...
def genValidSignature(message, index, userIDs, pk, sk, P, g):    
    signer = index % cyh.l # not including key in the sky
    print("signer: ", signer)
    sig = cyh.sign(userIDs[signer], userIDs, pk[signer], sk[signer], message)
    (Lt, pklist, u, S) = sig
    assert cyh.verify(Lt, pklist, P, g, message, u, S), "failed individual verification"
    return sig
...
def generate_signatures_main(argv, num_signer):
    ...
    pick_mpk = objectToBytes({ 'P':P, 'g':g }, group)
    ...
    sk = {}
    # number of signers
    cyh.l = num_signers # numValidMessages + 1 # add one to represent key in the sky
    userIDs = [ "test" + str(z) for z in range(cyh.l) ]
    for z in range(0, cyh.l):
        (pk[z], sk[z]) = cyh.keygen(alpha, userIDs[z])
    ...
    genOutputDictFile(numValidMessages, messageSize, 'mpk.charmPickle', 'pk.charmPickle', validOutputDict, validOutputDictName, '_ValidMessage.pythonPickle', '_ValidSignature.charmPickle', True, userIDs, pk, sk, P, g)
    genOutputDictFile(numInvalidMessages, messageSize, 'mpk.charmPickle', 'pk.charmPickle', invalidOutputDict, invalidOutputDictName, '_InvalidMessage.pythonPickle', '_InvalidSignature.charmPickle', False, userIDs, pk, sk, P, g)
    return
...
def run_batch_verification(argv):
    ...
    for programIteration in range(0, NUM_PROGRAM_ITERATIONS):
        ...
        for cycle in range(0, NUM_CYCLES):
            ...
            g = sigsDict[0]['mpk'][bodyKey]['g']
            Mlist  = [ sigsDict[i]['message'][bodyKey] for i in range(0, N) ]
            Lt     = sigsDict[0]['sig'][bodyKey][0]
            pklist = sigsDict[0]['sig'][bodyKey][1]
            ulist  = [ sigsDict[i]['sig'][bodyKey][2] for i in range(0, N) ]            
            Slist  = [ sigsDict[i]['sig'][bodyKey][3] for i in range(0, N) ]
            cyh.l = len(pklist)
            startTime = time.clock()
            incorrectSigIndices = cyh.batchverify(Lt, Mlist, P, Slist, g, pklist, ulist, [])
            endTime = time.clock()
            ...
            startTime = time.clock()
            incorrectSigIndices = cyh.indivverify(Lt, Mlist, P, Slist, g, pklist, ulist, [])
            endTime = time.clock()
