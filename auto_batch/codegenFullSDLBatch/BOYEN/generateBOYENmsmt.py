...
def genValidSignature(message, index, g1, g2, Alist, Blist, Clist, Atlist, Btlist, Ctlist, sklist):    
    signer = index % (boyen.l-1) # not including key in the sky
    print("signer: ", signer)
    sig = boyen.sign(g1, Alist, Blist, Clist, sklist[signer], message, signer+1)
    (S, t) = sig
    assert boyen.verify(g1, g2, Atlist, Btlist, Ctlist, message, S, t), "failed individual verification"
    return sig
...
def generate_signatures_main(argv, num_signer):
    ...
    # 1. generate keys
    (mpk, g1, g2) = boyen.setup()
       
    f_mpk = open('mpk.charmPickle', 'wb')
    # 2. serialize the pk's
    pick_mpk = objectToBytes({ 'g1':g1, 'g2':g2 }, group)
    f_mpk.write(pick_mpk)
    f_mpk.close()
    
    Alist, Blist, Clist = {0:mpk[0]}, {0:mpk[1]}, {0:mpk[2]}
    ...
    for z in range(1, boyen.l):
        ...
        Atlist[z], Btlist[z], Ctlist[z] = pklist[3], pklist[4], pklist[5]
    
    f_pk = open('pk.charmPickle', 'wb')
    # 2. serialize the pk's
    pick_pk = objectToBytes( { 'Alist':Alist, 'Blist':Blist, 'Clist':Clist, 'Atlist':Atlist, 'Btlist':Btlist, 'Ctlist':Ctlist }, group)
    f_pk.write(pick_pk)
    f_pk.close()
    ...
    genOutputDictFile(numValidMessages, messageSize, 'mpk.charmPickle', 'pk.charmPickle', validOutputDict, validOutputDictName, '_ValidMessage.pythonPickle', '_ValidSignature.charmPickle', True, g1, g2, Alist, Blist, Clist, Atlist, Btlist, Ctlist, sklist)
...
def run_batch_verification(argv):
    ...
    for programIteration in range(0, NUM_PROGRAM_ITERATIONS):
        ...
        for cycle in range(0, NUM_CYCLES):
            ...
            Btlist = sigsDict[0]['pk'][bodyKey]['Btlist']            
            Ctlist = sigsDict[0]['pk'][bodyKey]['Ctlist']            
            ...
            startTime = time.clock()
            incorrectSigIndices = boyen.indivverify(Atlist, Btlist, Ctlist, Mlist, Slist, g1, g2, tlist, [])
            endTime = time.clock()
