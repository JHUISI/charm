...
def genValidSignature(message, index, mpk, pklist, sklist, ilist):
    sig = hwdiff.sign(mpk, pklist[index], sklist[index], ilist[index], message)
    (sig1, sig2, r, ilist[index]) = sig

    g1, g2, w1, w2, z1, z2, h1, h2, u, v, d = mpk
    U, V, D = pklist[index]
    assert hwdiff.verify(U, V, D, g2, w2, z2, h2, message, sig1, sig2, r, ilist[index]), "failed verification"
    return sig
...
def generate_signatures_main(argv, same_signer=True):
    ...
    sklist = {}
    ilist = {}
    g2, u, v, d = mpk[1], mpk[8], mpk[9], mpk[10]
    for z in range(0, numValidMessages):
        (ilist[z], pklist[z], sklist[z]) = hwdiff.keygen(g2, u, v, d)
    
    f_mpk = open('mpk.charmPickle', 'wb')
    # 2. serialize the pk's
    pick_mpk = objectToBytes({ 'mpk':mpk }, group)
    ...
    genOutputDictFile(numValidMessages, messageSize, 'mpk.charmPickle', 'pk.charmPickle', validOutputDict, validOutputDictName, '_ValidMessage.pythonPickle', '_ValidSignature.charmPickle', True, mpk, pklist, sklist, ilist)
    genOutputDictFile(numInvalidMessages, messageSize, 'mpk.charmPickle', 'pk.charmPickle', invalidOutputDict, invalidOutputDictName, '_InvalidMessage.pythonPickle', '_InvalidSignature.charmPickle', False, mpk, pklist, sklist, ilist)
    return
...
def run_batch_verification(argv, same_signer=True):
    ...
    for programIteration in range(0, NUM_PROGRAM_ITERATIONS):
        ...
        for cycle in range(0, NUM_CYCLES):
            ...
            g1, g2, w1, w2, z1, z2, h1, h2, u, v, d = sigsDict[0]['mpk'][bodyKey]['mpk'][:]            
            Ulist = [ sigsDict[i]['pk'][bodyKey][i][0] for i in range(0, N) ]
            Vlist = [ sigsDict[i]['pk'][bodyKey][i][1] for i in range(0, N) ]
            Dlist = [ sigsDict[i]['pk'][bodyKey][i][2] for i in range(0, N) ]
            ...
            incorrectSigIndices = hwdiff.batchverify(Dlist, Ulist, Vlist, g2, h2, ilist, Mlist, rlist, sig1list, sig2list, w2, z2, [])
            endTime = time.clock()
            ...
            startTime = time.clock()
            incorrectSigIndices = hwdiff.indivverify(Dlist, Ulist, Vlist, g2, h2, ilist, Mlist, rlist, sig1list, sig2list, w2, z2, [])
            endTime = time.clock()
