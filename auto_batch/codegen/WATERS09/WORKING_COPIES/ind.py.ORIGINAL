...
def run_Ind(verifyArgsDict, groupObjParam, verifyFuncArgs):
	...
	for z in range(0, N):
		...
		E1=(( verifyArgsDict[z]['mpk'][bodyKey][ 'u' ] ** M ) *( verifyArgsDict[z]['mpk'][bodyKey][ 'w1' ] * tagc ) * verifyArgsDict[z]['mpk'][bodyKey][ 'h1' ] ) ** t
		...
		rhs_pair= pair(( verifyArgsDict[z]['mpk'][bodyKey][ 'tau1' ] ** s1 ) *( verifyArgsDict[z]['mpk'][bodyKey][ 'tau2' ] ** s2 ) , sig6 ) * pair(( verifyArgsDict[z]['mpk'][bodyKey][ 'tau1^b' ] ** s1 ) *( verifyArgsDict[z]['mpk'][bodyKey][ 'tau2^b' ] ** s2 ) *( verifyArgsDict[z]['mpk'][bodyKey][ 'w1' ] ** -t ) , sig7 ) *(( pair( E1 , sig7 ) / pair( E2 , sigK ) ) ** theta ) *( A ** s2 )
