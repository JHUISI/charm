...
class IBEWaters09(IBEnc):
    ...
    def verify(self, mpk, sigma, m):
        ...
        E1 = (mpk['u2'] ** (M * t)) * (mpk['w2'] ** (tagc * t)) * (mpk['h2'] ** t)
        E2 = mpk['g1'] ** t
        A = mpk['egg_alpha']
        theta =  ~(tagc - tagk)
        
        lhs_pair = pair(mpk['g1^b'] ** s, sig1) * pair(mpk['g^ba1'] ** s1, sig2) * pair(mpk['g^a1'] ** s1, sig3) * pair(mpk['g^ba2'] ** s2, sig4) * pair(mpk['g^a2'] ** s2, sig5)        
        rhs_pair = pair(sig6, (mpk['tau1'] ** s1) * (mpk['tau2'] ** s2)) * pair(sig7, (mpk['tau1^b'] ** s1) * (mpk['tau2^b'] ** s2) * (mpk['w2'] ** -t)) * (( pair(sig7, E1) / pair(E2, sigK) ) ** theta) * (A ** s2)  
