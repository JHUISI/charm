...
def verifySigsRecursive(verifyArgsDict, groupObj, incorrectIndices, startSigNum, endSigNum, delta, dotA, dotB, dotC, dotD, dotE, dotF, dotG):
	...
	if ( ( pair( dotA_loopVal , verifyArgsDict[z]['pk'][bodyKey] [ 'U_t' ] ) * pair( dotB_loopVal , verifyArgsDict[z]['pk'][bodyKey] [ 'g2' ] ) )==( dotC_loopVal *( pair( dotD_loopVal , verifyArgsDict[z]['pk'][bodyKey] [ 'h' ] ) * pair( dotE_loopVal , verifyArgsDict[z]['pk'][bodyKey] [ 'U2' ] [ 0 ] ) ) )  ):
		pass
	else:
