...
def run_Batch(verifyArgsDict, groupObjParam, verifyFuncArgs, toSort):
	...
	sortedSigEntries = {}
	for z in range(0, N):
		sortVal = verifyArgsDict[z][param][bodyKey]
		if (sortVal in sigNosMap):
			sigNosMap[sortVal].append(z)
			lenCurrentSigsBatch = len(sortedSigEntries[sortVal])
			sortedSigEntries[sortVal][lenCurrentSigsBatch] = verifyArgsDict[z]
		else:
			sigNosMap[sortVal] = []
			sigNosMap[sortVal].append(z)
			sortedSigEntries[sortVal] = {}
			sortedSigEntries[sortVal][0] = verifyArgsDict[z]
	...
	for sortValKey in sortedSigEntries:
		...
		for incorrect in incorrectsFromSortedBatch:
			incorrectIndices.append(sigNosMap[incorrect])
