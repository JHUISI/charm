...
def verifySigsRecursive(verifyArgsDict, groupObj, incorrectIndices, startSigNum, endSigNum, A, delta, dotA, dotB, dotD, sumE):
	z = 0
	...
	for y in range(0, l):
		...
		dotF_loopVal = dotF_loopVal *   pair( dotC_loopVal , verifyArgsDict[z]['mpk'][bodyKey] [ 'ub' ] [ y ] )  
	...
	if ( ( pair( dotA_loopVal , verifyArgsDict[z]['mpk'][bodyKey] [ 'g2' ] ) *(( pair( dotB_loopVal , verifyArgsDict[z]['mpk'][bodyKey] [ 'u1b' ] ) * dotF_loopVal ) * pair( dotD_loopVal , verifyArgsDict[z]['mpk'][bodyKey] [ 'u2b' ] ) ) )== A ** sumE_loopVal   ):
		return
	else:
		...
		verifySigsRecursive(verifyArgsDict, group, incorrectIndices, startSigNum, midSigNum, A, delta, dotA, dotB, dotD, sumE)
		verifySigsRecursive(verifyArgsDict, group, incorrectIndices, midSigNum, endSigNum, A, delta, dotA, dotB, dotD, sumE)
