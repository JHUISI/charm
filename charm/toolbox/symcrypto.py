...
class MessageAuthenticator(object):
    ...
    def mac(self, msg, additionalData=''):
        """
        ...
        """
        # Ensure the additional data is in byte format, convert if necessary. Really does nothing in python 2.x.
        if type(additionalData) != bytes :
            additionalData = bytes(additionalData)
        return {
                "alg": self._algorithm,
                "msg": msg,
                "digest": hmac.new(self._key, self._algorithm + additionalData + msg, digestmod=sha2).hexdigest()
               }

    def verify(self, msgAndDigest, additionalData=''):
        """
        ...
        if msgAndDigest['alg'] != self._algorithm:
            raise ValueError("Currently only HMAC_SHA2 is supported as an algorithm")
        expected = self.mac(msgAndDigest['msg'], additionalData=additionalData)['digest']
        received = msgAndDigest['digest']
...
class AuthenticatedCryptoAbstraction(SymmetricCryptoAbstraction):
    ...
    Traceback (most recent call last):
      ...
      File "./charm/toolbox/symcrypto.py", line 233, in decrypt
        raise ValueError("Invalid mac. Your data was tampered with or your key is wrong")
    ValueError: Invalid mac. Your data was tampered with or your key is wrong
    >>> cipher.decrypt(ciphertextAdditionalData, additionalData='\x10\x11\x11\x11')
    'Some network PDU.'
    >>>
    """
